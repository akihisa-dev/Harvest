import { galleryLinkScore, imageGroupLabel, normalizeImageUrls } from "../core/images.js";
import { createPdf } from "../core/pdf.js";
const source = required("#source-tab");
...
function renderGroups() {
    ...
    for (const [label, count] of groups) {
        ...
        if (images.some(item => imageGroupLabel(item.url) === label && item.selected))
...
async function toPdfPage(imageUrl) {
    ...
    try {
        ...
        const jpeg = await new Promise((resolve, reject) => canvas.toBlob(result => result ? resolve(result) : reject(new Error("画像を変換できませんでした。")), "image/jpeg", 0.95));
...
async function exportPdf() {
    ...
    try {
        for (const [index, image] of selected.entries()) {
            ...
            try {
                pages.push(await toPdfPage(image.url));
            }
            catch {
                failed += 1;
            }
...
void (async () => {
    ...
    if (!tabs.length)
        setStatus("収集できるWebページのタブがありません。");
